...
Suppose $\angle ABC\cong \angle ACB$. Then there is an isometry that reflects B from one side of D to the other and superimposes $\angle ABC$ onto $\angle ACB$. So let $f$ be that isometry $f(\angle ABC)=\angle ACB$. Isometries preserve distance. Therefore, by substitution $AB=AC$. 
...
```{r, echo = F, out.width = "35%", fig.align='center', fig.cap='A parallelogram ABCD'}
...
Then by triangle congruence for Side-Angle-Side we also know $\triangle ADC\cong \triangle DAB$, which implies that $\overline{AC}\cong\overline{BD}$. 
...
Our given assumption is $\overline{AC}\cong\overline{BD}$, so by triangle congruence theorem side-side-side $\triangle ADC \cong \triangle DAB\Rightarrow m\angle ADC = m\angle DAB$. 
